...
test_that("gen_col_control correctly retains old parameters", {
  ...
  expect_error(ctrl1 <- gen_col_control(agn_int_min = -1L), NA)
  ctrl2 <- gen_col_control(agn_int_max = 99L, old_ctrl = ctrl1)
  ctrl3 <- gen_col_control(agn_int_min = -2L, old_ctrl = ctrl1)
...
test_that("gen_col_control handles vector parameters", {
  ...
  expect_error(
    ctrl1 <- gen_col_control(agn_int_min = c(-1L, -2L), agn_int_max = 99L),
    NA
  )
