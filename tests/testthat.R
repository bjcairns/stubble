...
lna <- list(
  ...
  IDate = if (is.installed.package("data.table")) data.table::as.IDate(NA_character_) else NULL,
  integer = NA_integer_,
  integer64 = if (is.installed.package("bit64")) bit64::as.integer64(NA_integer_) else NULL,
  logical = NA,
...
luniq <- list(
  ...
  IDate = if (is.installed.package("data.table")) data.table::as.IDate(1:1e2, origin = "1970-01-01") else NULL,
  integer = 1:1e2,
  integer64 = if (is.installed.package("bit64")) bit64::as.integer64(0) else NULL,
  logical = c(F, T),
